...
namespace T4AzureArmTemplateGenerator.Namespace.Output
{
	public class NamespaceRootOutputParser
	{
				...
				foreach (var namespaceTargetLocation in namespaceInput.NamespaceTargetLocations)
				{
					...
					foreach (string namespaceEnvironment in namespaceTargetLocation.NamespaceEnvironments)
					{
						...
						newNamespace.Name = GetNamespaceName(namespaceInput.NamespaceNamePrefix, locationDefinition.NamespaceNameSuffix,
							namespaceEnvironment, input.NamespaceNameEndSuffix);
		...
		private string GetNamespaceName(string namespaceNamePrefix, string namespaceNameSuffix, string namespaceEnvironmentName, string namespaceNameEndSuffix)
		{
		    string name = $"{namespaceNamePrefix}-{namespaceNameSuffix}-{namespaceEnvironmentName}";

            if (string.IsNullOrWhiteSpace(namespaceNameEndSuffix))
			    return name;

		    return $"{name}-{namespaceNameEndSuffix}";
